...
const Appointment = () => {
  ...
  useEffect(() => {
    if (formData.doctor && formData.date) {
      axios
        .get(
          `http://localhost:8081/api/v1/appointments/${formData.doctor}/slots`
        )
        .then((response) => {
      ...
      axios
        .get(
          `http://localhost:8081/api/v1/appointments/check-locked-slots?doctorId=${formData.doctor}&date=${formData.date}`
        )
        .then((response) => {
          const lockedSlots = response.data;
          const available = timeSlots.filter(
            (slot) => !lockedSlots.includes(slot.value)
          );
  ...
  const handleTimeSlotChange = (slot) => {
    axios
      .post("http://localhost:8081/api/v1/appointments/lock-slot", {
      ...
      .catch((error) => {
        ...
        $(".time-slots").append(
          '<span class="time-error">This time slot is already taken, please choose another one.</span>'
        );
        setTimeout(function () {
          $(".time-error").remove();
        }, 2000);
